...
# 全局变量
model_path = '../models/face_expression_multiMood.hdf5'
...
if not input_video:
    camera = cv2.VideoCapture('../images/tests/emotion.mp4')
...
while True:
    ...
    for (left, top, right, bottom) in face_location_list:
        ...
        label = labels[model.predict(roi)[0].tolist().index(max(model.predict(roi)[0].tolist()))]
